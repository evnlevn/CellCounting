...
if Modulate.Preprocessing == 1
    ...
    for i=1:64:size(im,2)
        ...
        subplot(2,1,1)
        imagesc(squeeze(im(:,:,i))); colormap jet; axis image;
        title(sprintf('Image of Slice #%d',i));
        subplot(2,1,2)
        histogram(im(:,:,i));  title('Histogram of Pixel Intensities');
    end
    %
end
%% K-means Clustering
...
if Modulate.K_Means == 1
    ...
    im = double(im); % Converting Resized Image Datatype to double
    [thickness,x,y] = size(im);
    % Initialize Matrix
    cluster = zeros(thickness,x,y);
    
    % Engaging the K-means function on each slice
    for i = 1:size(im,2)
        aa = squeeze(im(:,:,i)); % Reduce dimensions to 2
        ...
        idx = reshape(idx,[thickness,size(im,2)]); % Reshaping data into original matrix format
    ...
    for i = 1:z
        for j = 1:x
            for k = 1:y
                win(i,j,k) = im(i,j,k).*cluster(i,j,k);
            end
        end
    end
