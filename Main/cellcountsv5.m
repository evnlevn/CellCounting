...
if Modulate.ReadDat == 1
    ...
    file=dir('C:\Users\Marien\Downloads\CellCounting-master\CellCounting-master\Main\20180423T173612\*.dat');
    data=zeros(size(intensity));
    for i=1:length(file)
        fname=strcat('C:\Users\Marien\Downloads\CellCounting-master\CellCounting-master\Main\20180423T173612\',file(i).name);
...
if Modulate.K_Means == 1
    ...
    for i = 1:size(rsz,2)
        ...
        idx = reshape(idx,[thickness,size(rsz,2)]); % Reshaping data into original matrix format
    ...
    count = 1;
    increment = 32;
    for i = increment:increment:size(rsz,2)
        subplot(8,2,count)
